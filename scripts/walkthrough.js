...
async function runSelfTest() {
  ...
  selfTestPromise = (async () => {
    ...
    try {
      ...
      curRoom = room;
      openEd(room);
      const wall0 = curRoom.walls[0],
        wall1 = curRoom.walls[1];
      ...
      note(
        "door created",
        curRoom.openings.some((o) => o.type === "door"),
      );
      note("window created", curRoom.openings.filter((o) => o.type === "window").length === 2);
      note(
        "closet created",
        curRoom.structures.some((s) => s.type === "closet" && s.finish === "white_shaker"),
      );
      sel = { type: "structure", idx: 0 };
      uS("finish", "dark_walnut");
      note("closet finish options work", curRoom.structures[0].finish === "dark_walnut");
      sel = { type: null, idx: -1 };
      note(
        "picker keys match placed asset keys",
        placedAssets.every((item) =>
          curRoom.furniture.some((f) => f.assetKey === item.assetKey && f.label === item.label),
        ),
      );
      sel = { type: "opening", idx: 1 };
      panelHidden = false;
      showP();
      note(
        "window panel opens",
        document.getElementById("propsP").innerHTML.includes("Window") &&
          document.getElementById("propsP").innerHTML.includes("props-close"),
      );
      closeP();
      sel = { type: "opening", idx: 1 };
      panelHidden = false;
      ...
      setTrimColor(TRIM_COLORS[2]);
      setCeilingBrightness(1.2);
      setLightingPreset("warm_evening");
      note(
        "room materials updated",
      ...
      note(
        "lighting preset affects renderer",
        Math.abs((ren?.toneMappingExposure || 0) - expectedLightingState.exposure) < 0.001,
        `exp:${ren?.toneMappingExposure}`,
      );
      note(
        "checker tile texture active",
        scene?.children.some(
          (ch) =>
            ch.material &&
            ch.material.map &&
            ch.material.map.repeat &&
            Math.abs(
              ch.material.map.repeat.x - FLOOR_TYPES.find((f) => f.id === "checker_tile").repeat,
            ) < 0.01,
        ),
      );
      const requiredRoomAssets = [
        "sofa",
        "bookshelf",
        "dining_table",
        "blinds",
      ...
      note(
        "room scene uses mapped models",
        requiredRoomAssets.every((k) => ROOM_MODEL_DEBUG.ok.has(k)),
        `room-ok:${[...ROOM_MODEL_DEBUG.ok].join(",")}`,
      );
      note(
        "room scene has no mapped fallback block",
        ROOM_MODEL_DEBUG.blocked.size === 0,
        `blocked:${[...ROOM_MODEL_DEBUG.blocked].join(",")}`,
      );
      note(
        "room scene has no mapped load fail",
        ROOM_MODEL_DEBUG.fail.size === 0,
        `fail:${[...ROOM_MODEL_DEBUG.fail].join(",")}`,
      );
      exit3DView();
      await sleep(200);
      ...
      note(
        "save/load persisted",
        !!reloaded &&
          reloaded.furniture.length === 9 &&
          reloaded.openings.length === 3 &&
          reloaded.structures.length === 1 &&
          reloaded.materials.floorType === "checker_tile" &&
      ...
      await sleep(600);
      note(
        "save/load preserved real models",
        requiredRoomAssets.every((k) => ROOM_MODEL_DEBUG.ok.has(k)) &&
          ROOM_MODEL_DEBUG.fail.size === 0 &&
          ROOM_MODEL_DEBUG.blocked.size === 0,
        `room-ok:${[...ROOM_MODEL_DEBUG.ok].join(",")}`,
      ...
      exit3DView();
      await sleep(100);
      await openAssetVerification();
      await sleep(5000);
      const loadedCards = [...document.querySelectorAll(".verify-card .verify-badge.loaded")]
        .length;
      const failedCards = [...document.querySelectorAll(".verify-card .verify-badge.fail")].length;
      note(
        "asset verification gallery loaded",
        loadedCards === Object.keys(MODEL_REGISTRY).length,
        `loaded:${loadedCards} failed:${failedCards}`,
      );
    } catch (err) {
...
async function boot() {
  ...
  await loadAll();
  await checkWelcome();
  if (DEV_MODE) await preflightAllModels();
  applyTimeTheme();
